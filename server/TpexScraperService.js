...
export async function fetchOTCListedStocks() {
    const decoder = new TextDecoder("big5");
    let url = 'https://isin.twse.com.tw/isin/class_main.jsp?market=2'
...
export async function fetchOTCEquitiesInstInvestorsTrades(date) {
    // `date` 轉換成 `民國年/MM/dd` 格式
    const dt = DateTime.fromISO(date);
    const year = dt.get('year') - 1911;
    const formattedDate = `${year}/${dt.toFormat('MM/dd')}`;
